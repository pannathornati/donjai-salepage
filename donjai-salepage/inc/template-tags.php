...
if ( ! function_exists( 'donjai_posted_on' ) ) :
	/**
	 * Prints HTML with meta information for the current post-date/time.
	 */
	function donjai_posted_on($show_icon = true) {
...
if ( ! function_exists( 'donjai_posted_by' ) ) :
	/**
	 * Prints HTML with meta information for the current author.
	 */
	function donjai_posted_by($show_icon = true) {
...
if ( ! function_exists( 'donjai_posted_cats' ) ) :
	/**
	 * Show Categories
	 */
	function donjai_posted_cats($show_icon = true) {
		if ( 'post' === get_post_type() ) {
			$categories_list = get_the_category_list( esc_html__( ', ', 'donjai' ) );
			if ( $categories_list ) {
...
if ( ! function_exists( 'donjai_posted_tags' ) ) :
	/**
	 * Show Tags
	 */
	function donjai_posted_tags() {
		if ( 'post' === get_post_type() ) {
...
if ( ! function_exists( 'donjai_entry_footer' ) ) :
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 */
	function donjai_entry_footer() {
		edit_post_link(
			sprintf(
				wp_kses(
					/* translators: %s: Name of current post. Only visible to screen readers */
					__( 'Edit <span class="screen-reader-text">%s</span>', 'donjai' ),
					array(
						'span' => array(
							'class' => array(),
						),
 ...
 */
function donjai_posts_navigation($wp_query = NULL) {
	if(!$wp_query) {
		global $wp_query; 
	}
    ...
    <a href="<?php echo $GLOBALS['s_member_url']; ?>" <?php
        ...
        <span class="pic">
            <?php 
				...
				if( 0 != $current_user->ID) { echo get_avatar($current_user->ID, 64 ); } else {  donjai_icon('s-user');} 
			?>
        </span>
        <span class="info">
            <?php 
				if($GLOBALS['s_member_label'] == 'Member') { _e( 'Member', 'donjai' ); } else {echo $GLOBALS['s_member_label'];} 
			?>
        </span>
    </a>
</div>
...
*/
function donjai_get_thumbnail($post_id) {
 ...
 */
function donjai_banner_title($post_id) {
	// Title Style
	$post_title_style = '';
	...
	if ($title_style == 'banner' ) {
		$banner_url = donjai_get_thumbnail($post_id);
	...
	if( is_front_page() ) { 
		...
		$title = __('Page not found', 'donjai' ); 
	} else { 
 ...
 */
function donjai_author($author_id) {
 ...
 */
if ( ! function_exists( 'donjai_icon' ) ) :
	function donjai_icon($i = NULL) {
		if(!$i) {
			return;
		}
