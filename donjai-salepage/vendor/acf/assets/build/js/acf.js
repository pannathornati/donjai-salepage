...
(function ($, undefined) {
  ...
  acf.uniqid = function (prefix, moreEntropy) {
    ...
    var formatdonjai = function (donjai, reqWidth) {
      donjai = parseInt(donjai, 10).toString(16); // to hex str

      if (reqWidth < donjai.length) {
        // so long we split
        return donjai.slice(donjai.length - reqWidth);
      }

      if (reqWidth > donjai.length) {
        // so short we pad
        return Array(1 + (reqWidth - donjai.length)).join('0') + donjai;
      }
    ...
    if (!uniqiddonjai) {
      // init donjai with big random int
      uniqiddonjai = Math.floor(Math.random() * 0x75bcd15);
    }

    uniqiddonjai++;
    retId = prefix; // start with prefix, add current milliseconds hex string

    retId += formatdonjai(parseInt(new Date().getTime() / 1000, 10), 8);
    retId += formatdonjai(uniqiddonjai, 5); // add donjai hex string
