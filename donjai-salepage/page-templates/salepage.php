...
if (get_field('show_header','option')) {
...
if ( !get_field('show_footer','option')) {
...
if ( !get_field('show_summary','option')) {
    echo '#sum{display:none}';
}

if (get_field('background_color','option')) {
    echo 'body.page-template-salepage{background-color:'. get_field('background_color','option') . '}';
}

if (get_field('accent_color','option')) {
    echo ':root{--sp-accent:'. get_field('accent_color','option') . '}';
}

echo '.sp-logo img{border-radius:'. get_field('rounded_logo','option') . 'px}';
...
<div class="page-salepage main-body">
    <div id="primary" class="content-area">
        <main id="main" class="site-main hide-title">
            <div class="sp-banner">
                <?php
				if (get_field('cover_m','option')) {
					echo wp_get_attachment_image(get_field('cover_m','option'), 'full', false, array('class' => 'mobile'));
				}
				if (get_field('cover_d','option')) {
					echo wp_get_attachment_image(get_field('cover_d','option'), 'full', false, array('class' => 'tablet'));
				}
				?>
            </div>
            <?php
				...
				echo wp_get_attachment_image(get_field('logo','option'), 'full');
				echo '</div>';
				}
			?>
            <div class="entry-content">
                <?php if(get_field('buy_now_button','option')) : ?>
                <div id="sp-buy" class="-sh">
                    <?php the_field('buy_now_text','option'); ?>
                </div>
                <?php endif; ?>
                <?php the_content(); ?>
                <div id="sp-start">
                    ...
                    <?php $table = get_field('table','option'); ?>
                    ...
                    <?php if (get_field('show_bank','option')) : // Bank Account ?>
                    <?php 
                        ...
                        echo __('Account No.', 'donjai') . ' <input type="text" id="acc-no" value="' . $bank['account_no'] . '" readonly style="width:'. $acc_num .'ch;"><i class="i-copy"></i>';
                        echo '<span class="acc-copied" aria-hidden="true">คัดลอกเรียบร้อย!</span>';
                        echo '</div>';
                        echo __('Account Name', 'donjai') . ' ' . $bank['account_name'];
                        echo '</div>';
                        echo '</div></div>';
                    ?>
                    ...
                    <div id="sp-form" class="sp-form <?php echo $form == 'success' ? 'success' : '' ;?>">
                        <?php
                            if (get_field('use_forminator','option')) {
                                echo do_shortcode('[forminator_form id="' . get_field('form','option') . '"]');
                            } elseif (get_field('other_plugin_shortcode','option')) {
                                echo do_shortcode(get_field('other_plugin_shortcode','option'));
                            }
                        ?>
